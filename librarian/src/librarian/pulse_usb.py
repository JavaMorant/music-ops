...
def _pdb_sessions(pdb: Path):
    ...
    for m in re.finditer(r"Track\(Track \{(.*?)\}\)", txt):
        ...
        if i and t:
            tr[int(i.group(1))] = (f"{artists.get(int(a.group(1)) if a else 0, '')} - {_unesc(t.group(1))}".strip(" -"),
                                   genres.get(int(g.group(1)) if g else 0, ""), 0.0)
    ...
    for pid in sorted(by_pl):
        ...
        for _, t in by_pl[pid]:
            if t in tr:
                meta[_norm(tr[t][0])] = {"genre": tr[t][1], "bpm": tr[t][2]}
        if seq:
            sessions.append(seq)
...
def _dlplus_sessions(db_path: Path):
    ...
    return [by_hist[h] for h in sorted(by_hist, key=int)], meta
...
def usb_insights(vol: Path, last_n: int = 50) -> dict:
    sessions, meta, fmts = read_stick(vol)
